...
The current `main` `CreateCampaign` and shared `MediaUpload` surfaces still accept and render video media (`video/*` and common video extensions). This conflicts with Issue #19's explicit no-video boundary. It is a discovered existing-path dependency, not an implementation of this plan. The eventual implementation must either narrow the campaign-cover/gallery caller to image-only while preserving any legitimately video-capable non-campaign callers, or otherwise demonstrate from repository evidence why the existing path must remain. Do not globally remove video support without auditing all `MediaUpload` callers.
...
4. Trace every `MediaUpload` caller before narrowing its accepted media types so unrelated profile/update functionality is not broken.
5. Determine whether the current storage/upload capability already supports multiple assets; if it does, extend that contract rather than creating a second storage path.
6. Identify owner/admin authorization boundaries for adding, replacing, reordering, and removing campaign media.
7. Confirm generated cover images remain distinct from user-uploaded campaign media and preserve the existing signature-style generation contract.
8. Establish bounded media constraints before coding: maximum asset count, accepted image types, maximum dimensions/size, safe failure behavior, and deterministic ordering.
9. Confirm deletion/replacement semantics do not remove a generated cover or another user's asset accidentally.
10. Check campaign-detail first-viewport requirements so multiple media assets do not push title, donate CTA, and description below the initial mobile viewport.
11. Define keyboard, screen-reader, touch, focus, loading, error, and empty-state behavior.
...
## Required workflow
1. Agent 2+3 review this plan against the actual repository schema/storage/callers, including the discovered existing video-capable path.
