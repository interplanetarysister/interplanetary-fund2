...
| Consolidate Funds / Count My Money | `FinancialManagement.tsx` has manual consolidate flow and reconciliation summary | no `FinancialManagement` or `Consolidate Funds` code-search hit on ifund2 main | VERIFIED MISSING SURFACE | implement Base44-compatible user-facing consolidation control backed by authoritative sync interface; do not copy Convex state |
