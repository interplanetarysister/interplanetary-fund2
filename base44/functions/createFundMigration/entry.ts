...
const ALLOWED_SOURCE_PLATFORMS = new Set(['GoFundMe', 'Kickstarter', 'Indiegogo', 'Facebook', 'GiveSendGo', 'CashApp', 'PayPal', 'Other']);
...
export default async function(req) {
  try {
    ...
    const { request_id: requestId, campaign_id: campaignId, source_platform: sourcePlatform, gross_amount: rawGrossAmount, payout_method: payoutMethod, payout_destination: payoutDestination } = body || {};
    ...
    if (!Number.isFinite(grossAmount) || grossAmount <= 0 || grossAmount > MAX_GROSS_AMOUNT) return Response.json({ error: 'Enter a valid migration amount.' }, { status: 400 });
    ...
    if (existing) return Response.json({ ok: true, duplicate: true, withdrawal_id: existing.id, status: existing.status, gross: existing.gross_amount, fee: existing.platform_fee, net: existing.net_amount });

    // The campaign record is the supported atomic single-winner boundary for a
    // migration request. A retry reuses its stable request_id; it never reclaims
    // an already-active migration, which prevents two identical submissions from
    // reaching Withdrawal.create concurrently. The claim remains until the
    // authoritative withdrawal approval/reconciliation workflow reaches a
    // terminal outcome, where that workflow must clear it conditionally.
    const claim = await sr.entities.Campaign.updateMany(
      { id: campaignId, active_migration_request_id: { $in: ['', null] } },
      { $set: { active_migration_request_id: requestId } },
    );

    if (!claim.success || claim.updated !== 1) {
      const current = await sr.entities.Campaign.get(campaignId);
      if (current?.active_migration_request_id === requestId) {
        return Response.json({ error: 'This migration request is already being processed. Retry reconciliation after the withdrawal request is visible.' }, { status: 409 });
      }
    ...
    if (net <= 0) {
      await sr.entities.Campaign.updateMany({ id: campaignId, active_migration_request_id: requestId }, { $unset: { active_migration_request_id: '' } });
