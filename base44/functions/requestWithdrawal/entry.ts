...
const UNKNOWN_PAYOUT_REVIEW_NOTE = 'Payout provider outcome is unknown. Funds remain held until deterministic reconciliation confirms a terminal provider result.';
...
async function clearMigrationClaim(sr, withdrawal) {
  ...
  if (!result?.success || result.updated !== 1) {
    console.error('requestWithdrawal migration claim reconciliation incomplete:', { campaign_id: withdrawal.campaign_id, migration_request_id: withdrawal.migration_request_id, withdrawal_id: withdrawal.id });
  }
}
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      const claim = await sr.entities.Withdrawal.updateMany(
        ...
        { $set: { status: 'processing', payout_claim_token: claimToken, payout_claimed_at: new Date().toISOString(), review_action: 'approve', reviewed_by_id: user.id, reviewed_at: new Date().toISOString() } },
      );
    ...
    if ((recent || []).some((w) => w.status !== 'failed' && new Date(w.created_date) >= startToday)) return Response.json({ error: 'You can only withdraw once per day. Please try again tomorrow.' }, { status: 400 });
