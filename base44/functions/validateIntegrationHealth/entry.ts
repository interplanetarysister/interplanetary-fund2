    ...
    if (String(e.account_identifier || '').toLowerCase().includes('platform-managed')) {
      ...
      try {
        ...
        if (conn && conn.accessToken) {
          checks.push({ check: 'oauth_configured', ok: true, detail: 'connector credential is configured; live provider verification unavailable' });
          status = mergeIntegrationStatus(status, 'DISCONNECTED');
          providerVerified = false;
          if (!lastFailure) lastFailure = 'Live provider verification is unavailable.';
        } else {
          checks.push({ check: 'oauth_configured', ok: false, detail: 'connector credential is not configured' });
...
export default async function(req) {
  try {
    ...
    for (const e of entries) {
      ...
      const isLegacyConvex = String(e.platform || '').toLowerCase() === 'convex';
      if (!isLegacyConvex && isUnhealthy(result.status)) {
