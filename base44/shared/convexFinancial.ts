...
async function campaignTotals(sr, campaignId) {
  ...
  return {
    raisedAmount: round2(donations.reduce((sum, r) => sum + giftOf({ amount: r.gross_amount, platform_contribution: r.platform_contribution }), 0)),
...
export async function recordCanonicalDonation(sr, args) {
  ...
  if (existing) {
    ...
    return { operationId: existing.id, applied: false, status: existing.status === 'pending' ? 'pending_duplicate' : 'duplicate', ...totals };
  }
  const verified = args.paymentVerified === true;
  const row = await sr.entities.FinancialOperation.create({
    operation_key: String(args.operationKey),
    operation_type: 'donation',
    campaign_id: String(args.campaignId),
    campaign_owner_user_id: String(args.campaignOwnerUserId || ''),
    provider: String(args.provider || args.paymentMethod || ''),
    provider_transaction_id: String(args.providerTransactionId || ''),
    gross_amount: Number(args.grossAmount || 0),
    platform_contribution: Number(args.platformContribution || 0),
    processing_fee: Number(args.processingFee || 0),
    payment_verified: verified,
    status: verified ? 'confirmed' : 'pending',
    metadata: {
      campaign_title: args.campaignTitle || '', donor_name: args.donorName || '', donor_email: args.donorEmail || '',
      donor_user_id: args.donorUserId || '', message: args.message || '', source: args.source || '', is_recurring: !!args.isRecurring,
    },
  });
  const totals = await campaignTotals(sr, args.campaignId);
...
export async function recordCanonicalExternalObservation(sr, args) {
  ...
  const row = await sr.entities.FinancialOperation.create({
    operation_key: String(args.operationKey), operation_type: 'external_observation', campaign_id: String(args.campaignId),
    campaign_owner_user_id: String(args.campaignOwnerUserId || ''), provider: String(args.provider || ''),
    provider_transaction_id: String(args.providerTransactionId || ''), gross_amount: Number(args.grossAmount || args.amount || 0),
    payment_verified: args.paymentVerified === true, status: 'observed', metadata: args,
  });
...
export async function reserveCanonicalWithdrawal(sr, args) {
  ...
  const available = round2(totals.raisedAmount - committed);
  if (requested > available) throw new Error('Requested withdrawal exceeds the verified available balance.');
  const { fee, net } = computeWithdrawal(requested);
  const reservationId = `b44:${String(args.operationKey)}`;
  const row = await sr.entities.FinancialOperation.create({
    operation_key: String(args.operationKey), operation_type: 'withdrawal', campaign_id: String(args.campaignId),
    campaign_owner_user_id: String(args.campaignOwnerUserId || ''), gross_amount: requested, platform_fee: Number(fee), net_amount: Number(net),
    status: 'reserved', reservation_id: reservationId, payout_method: String(args.payoutMethod || ''), payout_destination: String(args.payoutDestination || ''),
  });
...
export async function completeCanonicalWithdrawal(sr, args) {
  const row = await findOperation(sr, args?.operationKey);
  if (!row || row.operation_type !== 'withdrawal') throw new Error('Withdrawal reservation not found.');
  if (row.status === 'completed') return { operationId: row.id, duplicate: true };
  if (row.status !== 'reserved') throw new Error('Withdrawal reservation is not active.');
  await sr.entities.FinancialOperation.update(row.id, { status: 'completed', provider_transaction_id: String(args.providerTransactionId || ''), completed_at: new Date().toISOString() });
  return { operationId: row.id, completed: true };
}

export async function cancelCanonicalWithdrawal(sr, args) {
  const row = await findOperation(sr, args?.operationKey);
  if (!row || row.operation_type !== 'withdrawal') throw new Error('Withdrawal reservation not found.');
  if (row.status === 'completed') throw new Error('Completed withdrawal cannot be cancelled.');
  if (row.status === 'cancelled') return { operationId: row.id, duplicate: true };
  await sr.entities.FinancialOperation.update(row.id, { status: 'cancelled', cancelled_at: new Date().toISOString(), metadata: { ...(row.metadata || {}), cancellation_reason: args.reason || '' } });
  return { operationId: row.id, cancelled: true };
}

export async function mirrorCanonicalCampaignTotal(sr, campaignId, canonical) {
  const totals = canonical && Number.isFinite(Number(canonical.raisedAmount))
    ? { raisedAmount: Number(canonical.raisedAmount), donorCount: Number(canonical.donorCount || 0) }
    : await campaignTotals(sr, campaignId);
