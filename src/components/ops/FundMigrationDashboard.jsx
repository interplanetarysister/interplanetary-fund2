...
const emptyEntry = () => ({ requestId: makeRequestId(), campaignId: "", campaignTitle: "", sourcePlatform: "GoFundMe", grossAmount: "" });
...
// admin-only createFundMigration backend function. Each row owns one stable
// request identity so retries reuse the same server reconciliation key.
export default function FundMigrationDashboard() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const migration of billable) {
        if (!migration.campaignId || !migration.sourcePlatform) throw new Error("Each migration needs a campaign and source platform.");
        const response = await base44.functions.invoke("createFundMigration", {
          request_id: migration.requestId,
      ...
      setResult({ created, totalGross }); setStep("result");
    } catch (e) {
  ...
  if (step === "result") return (
    <div className="text-center py-8">
      ...
      <Button variant="ghost" className="mt-5 text-cyan-400 hover:text-cyan-300" onClick={() => { setStep("entries"); setMigrations([emptyEntry()]); setResult(null); setPayoutDest(""); }}>New migration</Button>
    </div>
  );
  ...
  if (step === "confirm") return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-2 text-sm">
        {billable.map((m, i) => <div key={m.requestId} className="flex justify-between text-slate-300"><span>{m.campaignTitle || m.campaignId} · {m.sourcePlatform}</span><span className="font-semibold">{fmt(m.grossAmount)}</span></div>)}
        <div className="pt-2 border-t border-white/10 space-y-1"><div className="flex justify-between text-xs text-slate-400"><span>Estimated platform fee</span><span>− {fmt(estimatedPlatformFee)}</span></div><div className="flex justify-between font-bold text-slate-100"><span>Estimated net</span><span>{fmt(estimatedNet)}</span></div><p className="text-[11px] text-slate-500">These are display estimates only. The authoritative server workflow recalculates the financial values.</p></div>
      ...
      <div className="flex gap-2"><Button variant="ghost" className="flex-1 text-slate-400" onClick={() => setStep("payout")} disabled={submitting}>Back</Button><Button className="flex-1 bg-cyan-400/20 border border-cyan-400/30 text-cyan-300 hover:bg-cyan-400/30" onClick={handleSubmit} disabled={submitting}>{submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : "Submit migration"}</Button></div>
    </div>
  );
  ...
  if (step === "payout") return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 pt-2"><Button variant="ghost" className="flex-1 text-slate-400" onClick={() => setStep("entries")}>Back</Button><Button className="flex-1 bg-cyan-400/20 border border-cyan-400/30 text-cyan-300 hover:bg-cyan-400/30" onClick={() => setStep("confirm")} disabled={!payoutDest.trim()}>Review →</Button></div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {loadingCampaigns ? <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 text-cyan-400 animate-spin" /></div> : <>
        {migrations.map((m, i) => <div key={m.requestId} className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-2">
