    ...
    <Link to={`/campaign/${campaign.id}`} className="group bg-white rounded-2xl border border-stone-200/70 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
      <div className="h-40 bg-stone-100 overflow-hidden">
        {campaign.cover_image_url ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-orange-100 via-amber-50 to-stone-100" />
        )}
      </div>
      <div className="p-5">
        <p className="text-[11px] font-medium uppercase tracking-wider text-orange-600 mb-1.5">{categoryLabels[campaign.category] || "Other"}</p>
