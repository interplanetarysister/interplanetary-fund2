...
export default function ConnectDialog({ platform, existing, aiAuthorized, open, onOpenChange, onSaved }) {
  ...
  const save = async () => {
    ...
    if (mode !== "manual" && !aiAuthorized) return setError("AI Publishing Authorization is required before enabling automation.");
    ...
    const data = {
      ...
      last_synced: now,
      last_error: "",
      history: [...(existing?.history || []), { at: now, event: existing ? "synced" : "connected", detail: existing ? "Details and totals updated" : `Connected ${platform.name}` }].slice(-30),
    };
    try {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md rounded-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5"><Label>AI automation for this destination</Label><Select value={form.automation_mode} onValueChange={(v) => set("automation_mode", v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{AUTOMATION_MODES.map((m) => <SelectItem key={m.value} value={m.value} disabled={m.value !== "manual" && !aiAuthorized}>{m.label}</SelectItem>)}</SelectContent></Select><p className="text-xs text-stone-400">{aiAuthorized ? AUTOMATION_MODES.find((m) => m.value === form.automation_mode)?.desc : "Accept the AI Publishing Authorization above to enable automation options."}</p></div>
          {error && <p role="alert" className="text-sm text-red-600">{error}</p>}
          <Button onClick={save} disabled={saving} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground h-11 rounded-xl">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : existing ? "Save changes" : "Connect"}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
