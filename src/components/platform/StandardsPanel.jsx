...
export default function StandardsPanel() {
  return (
    <div className="space-y-2">
      ...
      <Section icon={<Gauge className="w-5 h-5 text-orange-600" />} title="Engineering Principles">
      ...
      <Section icon={<Code2 className="w-5 h-5 text-orange-600" />} title="Naming Conventions">
      ...
      <Section icon={<GitBranch className="w-5 h-5 text-orange-600" />} title="Branch Strategy">
        <div className="grid sm:grid-cols-2 gap-3">
          {branches.map((b) => (
            <div key={b.branch} className="bg-white rounded-xl border border-stone-200/70 shadow-sm p-4">
              <p className="font-mono text-sm text-orange-700">{b.branch}</p>
      ...
      <Section icon={<GitCommit className="w-5 h-5 text-orange-600" />} title="Commit Standards">
      ...
      <Section icon={<Tag className="w-5 h-5 text-orange-600" />} title="Versioning — Semantic Versioning">
        <div className="bg-white rounded-2xl border border-stone-200/70 shadow-sm divide-y divide-stone-100">
          {versioning.map((v) => (
            <div key={v.part} className="flex items-center gap-3 px-5 py-3.5">
              <Badge className="bg-orange-600 text-white hover:bg-orange-600 font-mono">{v.part}</Badge>
      ...
      <Section icon={<CheckSquare className="w-5 h-5 text-orange-600" />} title="Code Review Checklist">
        <div className="grid sm:grid-cols-2 gap-x-6 gap-y-1.5">
          {codeReview.map((c) => (
            <p key={c} className="text-sm text-stone-600 flex gap-2"><span className="text-orange-500">✓</span>{c}</p>
          ))}
        </div>
      </Section>

      <Section icon={<ShieldCheck className="w-5 h-5 text-orange-600" />} title="Code Quality Gates">
      ...
      <Section icon={<Bot className="w-5 h-5 text-orange-600" />} title="AI-Assisted Development Standards">
        <div className="bg-white rounded-2xl border border-stone-200/70 shadow-sm divide-y divide-stone-100">
      ...
      <Section icon={<Gauge className="w-5 h-5 text-orange-600" />} title="Engineering Metrics">
        <div className="flex flex-wrap gap-1.5">
          {metrics.map((m) => (
      ...
      <Section icon={<Wrench className="w-5 h-5 text-orange-600" />} title="Developer Experience (DX) Platform">
        <div className="grid sm:grid-cols-2 gap-2">
          {dxPlatform.map((d) => (
            <p key={d} className="text-sm text-stone-600 flex gap-2"><span className="text-orange-500">›</span>{d}</p>
          ))}
        </div>
      </Section>
    </div>
