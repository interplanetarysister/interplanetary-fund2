...
export default function FraudControlPanel() {
  ...
  const safeMessage = (ok, text) => {
    if (!mountedRef.current) return;
    if (ok) { setSuccess(text); setError(""); }
    else { setError(SAFE_FRAUD_ERROR); setSuccess(""); }
  };
  ...
  const runSingleFlight = async (key, operation) => {
    ...
    catch { safeMessage(false); return false; }
  ...
  const load = async () => {
    ...
    try {
      ...
      setWithdrawals(asArray(w));
      setCampaigns(asArray(c));
    } catch {
      if (!mountedRef.current || generation !== loadGenerationRef.current) return;
      setWithdrawals([]);
      setCampaigns([]);
  ...
  const deny = async (w) => {
    if (!denyReason.trim()) { safeMessage(false); return; }
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        {withdrawals.length === 0 ? <div className="bg-white rounded-2xl border border-dashed border-stone-300 p-8 text-center text-stone-400 text-sm">No withdrawals pending review.</div> : <div className="space-y-3">{withdrawals.map((w) => <div key={w.id} className="bg-white rounded-2xl border border-stone-200/70 shadow-sm p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-stone-900">{money(w.gross_amount)} gross → <span className="text-emerald-700 font-semibold">{money(w.net_amount)} net</span></p><p className="text-xs text-stone-500 mt-0.5">{w.campaign_title || w.campaign_id} · {w.paypal_email}</p>{w.review_note && <p className="text-xs text-amber-600 mt-1">{w.review_note}</p>}</div><span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-semibold uppercase">Under Review</span></div>{denyTarget !== w.id ? <div className="flex gap-2 mt-3"><Button size="sm" className="flex-1 bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100" variant="ghost" onClick={() => approve(w)}><CheckCircle2 className="w-3.5 h-3.5 mr-1" /> Approve</Button><Button size="sm" className="flex-1 bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100" variant="ghost" onClick={() => setDenyTarget(w.id)}><XCircle className="w-3.5 h-3.5 mr-1" /> Deny</Button></div> : <div className="mt-3 space-y-2"><input type="text" placeholder="Reason for denial (required)" value={denyReason} onChange={(e) => setDenyReason(e.target.value)} className="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm text-stone-800 outline-none focus:ring-2 focus:ring-rose-300" /><div className="flex gap-2"><Button size="sm" variant="ghost" className="flex-1 text-stone-500" onClick={() => { setDenyTarget(null); setDenyReason(""); }}>Cancel</Button><Button size="sm" className="flex-1 bg-rose-600 text-white hover:bg-rose-700" onClick={() => deny(w)} disabled={!denyReason.trim()}>Confirm Denial</Button></div></div>}</div>)}</div>}
      </section>
      <section>
