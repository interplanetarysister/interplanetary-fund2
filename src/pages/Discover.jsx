...
export default function Discover() {
  ...
  if (error && !campaigns) {
    return <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10"><PageError message={SAFE_DISCOVER_ERROR} onRetry={() => { setError(null); setRefreshKey((k) => k + 1); }} /></div>;
  }
  if (!campaigns) {
  ...
  return (
    <PullToRefresh onRefresh={() => setRefreshKey((k) => k + 1)} className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      ...
      {error && <div role="alert" className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{SAFE_DISCOVER_ERROR} <button type="button" className="underline" onClick={() => setRefreshKey((k) => k + 1)}>Retry</button></div>}
