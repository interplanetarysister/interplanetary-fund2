...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setError(""); if (newPassword !== confirmPassword) { setError("Passwords do not match"); return; } setLoading(true); try { await base44.auth.resetPassword({ resetToken, newPassword }); window.location.href = "/login"; } catch { setError(getSafeAuthError("Failed to reset password")); } finally { setLoading(false); } };
