...
const required = [
  ...
  ['workflow uses canonical admin role constant', workflow.includes("const ADMIN_ROLE = 'admin';")],
  ['workflow checks authenticated user role', workflow.includes('user.role !== ADMIN_ROLE')],
  ['User schema declares admin as the role enum', /"role"[\s\S]*?"enum"\s*:\s*\[[\s\S]*?"admin"[\s\S]*?"user"/.test(userSchema)],
  ...
  ['workflow rejects invalid role', workflow.includes("body.role === ADMIN_ROLE ? ADMIN_ROLE : body.role === 'user' ? 'user' : null")],
