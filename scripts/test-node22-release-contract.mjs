...
expect('commented declaration ignored', (root) => writeFileSync(join(root, '.github', 'workflows', 'quality.yml'), '# node-version: 24\n'), (errors) => errors.some((error) => error.includes('active node-version')) && !errors.some((error) => error.includes('pins node-version 24')));
expect('block scalar declaration ignored', (root) => writeFileSync(join(root, '.github', 'workflows', 'quality.yml'), 'description: |\n  node-version: 24\n'), (errors) => errors.some((error) => error.includes('active node-version')) && !errors.some((error) => error.includes('pins node-version 24')));
expect('active incompatible pin rejected', (root) => writeFileSync(join(root, '.github', 'workflows', 'quality.yml'), 'node-version: 24\n'), (errors) => errors.some((error) => error.includes('pins node-version 24')));
