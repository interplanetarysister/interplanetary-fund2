...
const required = [
  ...
  ['malformed collections fail closed', source.includes('normalizeRows(value, "invalid-collection")')],
  ['malformed donations fail closed', source.includes('normalizeRows(data?.donations, "invalid-donations")')],
  ['invalid rows are not silently filtered', source.includes('value.some((row) => !isRecord(row))')],
