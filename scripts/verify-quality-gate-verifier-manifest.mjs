...
if (!fs.existsSync(workflowPath)) fail('quality workflow file is missing');
...
for (const entry of manifest.commands) {
  if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
    fail('every command entry must be a plain object');
    continue;
  }
  ...
  if (typeof source !== 'string' || !source || seenSources.has(source) || !fs.existsSync(path.join(root, source))) {
    fail(`duplicate or missing verifier source for ${script}: ${String(source)}`);
  }
