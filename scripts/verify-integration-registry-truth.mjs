...
rejectText(health, "nextStatus === 'ACTIVE'", "token presence cannot become provider verification");
requireText(health, "live provider verification unavailable", "configured token remains fail closed");
requireText(health, "providerVerified = false", "token presence cannot advance successful verification");
requireText(health, "!isLegacyConvex && isUnhealthy(result.status)", "legacy Convex rows cannot emit active health alerts");
rejectText(health, "status = 'ACTIVE';\n    lastFailure = ''", "legacy Convex status is not fabricated active");
